...
class TestSQLiteDatabase:
    ...
    def setup_method(self, method):
        """Setup test instance"""
        with patch('database.sqlite_db.DatabaseBase.__init__') as mock_init:
            mock_init.return_value = None
            self.db = SQLiteDatabase()

    @patch('database.sqlite_db.DatabaseBase.get_session')
    ...
    @patch('database.sqlite_db.DatabaseBase.batch_insert')
    ...
    def test_add_index_existing(self, mock_get_session):
        """Test adding existing index returns None"""
        mock_session = MagicMock()
        mock_get_session.return_value = mock_session
    ...
    @patch('database.sqlite_db.DatabaseBase.get_session')
    def test_add_sector_success(self, mock_get_session):
        """Test adding new sector successfully"""
        mock_session = MagicMock()
        mock_get_session.return_value = mock_session

        # Mock no existing sector
        mock_session.query.return_value.filter.return_value.first.return_value = None
    ...
    @patch('database.sqlite_db.DatabaseBase.get_session')
    def test_add_sector_existing(self, mock_get_session):
        """Test adding existing sector returns None"""
        mock_session = MagicMock()
        mock_get_session.return_value = mock_session

        # Mock existing sector
        existing_sector = MagicMock()
        mock_session.query.return_value.filter.return_value.first.return_value = existing_sector
    ...
    @patch('database.sqlite_db.DatabaseBase.get_session')
    def test_get_all_stocks(self, mock_get_session):
        ...
        result = self.db.get_all_stocks()

        assert result == mock_stocks
        mock_session.close.assert_called_once()

    @patch('database.sqlite_db.DatabaseBase.get_session')
    def test_get_all_sectors(self, mock_get_session):
        """Test getting all sectors"""
        mock_session = MagicMock()
        mock_sectors = [MagicMock(), MagicMock()]
        mock_get_session.return_value = mock_session
        mock_session.query.return_value.all.return_value = mock_sectors

        result = self.db.get_all_sectors()

        assert result == mock_sectors
        mock_session.close.assert_called_once()