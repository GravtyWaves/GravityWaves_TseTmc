def Store_All_Data_To_DB():
    """
    ذخیره‌سازی اولیه کل داده‌های موجود در دیتابیس (SQLite و PostgreSQL)
    این تابع تمام داده‌های قابل جمع‌آوری را یکجا واکشی و ذخیره می‌کند.
    """
    try:
        # MarketWatch
        print("در حال ذخیره MarketWatch ...")
        Get_MarketWatch(save_excel=False)

        # 60D Price History
        print("در حال ذخیره PriceHistory60D ...")
        # فرض بر این است که لیست نمادها از Build_Market_StockList قابل دریافت است
        stock_list = Build_Market_StockList(detailed_list=True, show_progress=False, save_excel=False, save_csv=False)
        if isinstance(stock_list, pd.DataFrame):
            stock_names = stock_list.index.tolist()
        else:
            stock_names = stock_list
        Get_60D_PriceHistory(stock_names, adjust_price=True, show_progress=False, save_excel=False)

        # Shareholders info (اختیاری، اگر مدل و ذخیره‌سازی آماده باشد)
        print("در حال ذخیره اطلاعات سهامداران ...")
        for ticker in stock_names:
            try:
                df_sh = Get_ShareHoldersInfo(ticker)
                if df_sh is not None and not df_sh.empty:
                    ...
                    for idx, row in df_sh.reset_index().iterrows():
                        record = Shareholder(
                            ...
                            market=row['Market'],
                            name=row['Name'],
                            share_no=row['ShareNo'],
                            share_pct=row['SharePct'],
                            changes=row['Changes']
                        )
                        shareholder_records.append(record)
                ...
                print(f"خطا در ذخیره سهامداران {ticker}: {e}")

        print("ذخیره‌سازی اولیه داده‌ها در دیتابیس با موفقیت انجام شد.")
    except Exception as e:
        print(f"خطا در ذخیره‌سازی اولیه داده‌ها: {e}")
# imports:
import pandas as pd
import numpy as np
...
################################################################################################################################################################################
################################################################################################################################################################################
def get_tse_webid(stock:str = 'پترول', adjust_price:bool = False, show_weekday:bool = False, double_date:bool = False, ignore_date:bool = False, start_date=None, end_date=None) -> pd.DataFrame:
    """
    ...
    # Define get_price_data function here
    ...
    def srch_req(srch_key):
        ...
        srch_res = srch_res.set_index(['Ticker', 'Active'])
        return srch_res

    # search for WebIDs
    ticker_no_df = srch_req(stock)
...
def __Get_TSE_Sector_WebID__(sector_name):
    """
    Returns the sector web ID for a given sector name.
    You should implement the mapping from sector names to web IDs here.
    """
    sector_map = {
        'خودرو': 32097828799138957,
        # Add other sector names and their web IDs here
    }
    return sector_map.get(sector_name, None)
...
def Get_LCI30_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date = False, just_adj_close = False, show_weekday = False, double_date = False):
    ...
    # DATABASE STORAGE: Store LCI30 Index History in SQLite and PostgreSQL
    try:
        from database.models import IndexHistory
        from database.sqlite_db import get_sqlite_session
        from database.postgres_db import get_postgres_session
        index_records = []
        for jdate, row in df_sector_cl.iterrows():
            index_records.append(IndexHistory(
                index_name='LCI30',
                jdate=jdate,
        ...
        # Store in SQLite
        sqlite_session = get_sqlite_session()
        sqlite_session.bulk_save_objects(index_records)
        sqlite_session.commit()
        sqlite_session.close()
        # Store in PostgreSQL
        postgres_session = get_postgres_session()
        postgres_session.bulk_save_objects(index_records)
        postgres_session.commit()
        postgres_session.close()
    except Exception as e:
        print(f"Database storage error (LCI30 Index): {e}")
    if(not just_adj_close):
        r = requests.get(f'http://old.tsetmc.com/tsev2/chart/data/IndexFinancial.aspx?i={sector_web_id}&t=ph', headers=headers)
        df_sector = pd.DataFrame(r.text.split(';'))
        columns=['Date','High','Low','Open','Close','Volume','D']
...
def Get_ACT50_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date = False, just_adj_close = False, show_weekday = False, double_date = False):
    ...
    # DATABASE STORAGE: Store ACT50 Index History in SQLite and PostgreSQL
    try:
        from database.models import IndexHistory
        from database.sqlite_db import get_sqlite_session
        from database.postgres_db import get_postgres_session
        ...
        print(f"Database storage error (ACT50 Index): {e}")
    if(not just_adj_close):
        r = requests.get(f'http://old.tsetmc.com/tsev2/chart/data/IndexFinancial.aspx?i={sector_web_id}&t=ph', headers=headers)
        df_sector = pd.DataFrame(r.text.split(';'))
        columns=['Date','High','Low','Open','Close','Volume','D']
...
def Get_MarketWatch(save_excel = True, save_path = 'D:/FinPy-TSE Data/MarketWatch'):
    try:
        ...
        # add trade types:
        final_df['Trade Type'] = final_df['Ticker'].apply(lambda x: 'تابلو' if((not x[-1].isdigit())or(x in ['انرژی1','انرژی2','انرژی3'])) 
        ...
        #--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # DATABASE STORAGE: Store MarketWatch data in SQLite and PostgreSQL
        try:
            from database.sqlite_db import get_sqlite_session
            from database.postgres_db import get_postgres_session
            from database.models import MarketWatch
            # Convert DataFrame to list of MarketWatch ORM objects
            marketwatch_records = []
            for idx, row in final_df.reset_index().iterrows():
                ...
                marketwatch_records.append(record)
            # Store in SQLite
            sqlite_session = get_sqlite_session()
        ...
        print(f"Error in Get_MarketWatch: {e}")
        return None, None
################################################################################################################################################################################
################################################################################################################################################################################
...
def Build_PricePanel(stock_list, param = 'Adj Final', jalali_date = True, save_excel = True, save_path = 'D:/FinPy-TSE Data/Price Panel/'):
    ...
    # DATABASE STORAGE: Store Price Panel data in SQLite and PostgreSQL
    try:
        from database.models import PriceHistory
        from database.sqlite_db import get_sqlite_session
        from database.postgres_db import get_postgres_session
        pricehistory_records = []
        for jdate, row in df_panel.iterrows():
            for ticker in df_panel.columns:
                value = row[ticker]
                pricehistory_records.append(PriceHistory(
                    ticker=ticker,
                    jdate=jdate,
                    value=value
                ))
        # Store in SQLite
        sqlite_session = get_sqlite_session()
        sqlite_session.bulk_save_objects(pricehistory_records)
        sqlite_session.commit()
        sqlite_session.close()
        # Store in PostgreSQL
        postgres_session = get_postgres_session()
        postgres_session.bulk_save_objects(pricehistory_records)
        postgres_session.commit()
        postgres_session.close()
    except Exception as e:
        print(f"Database storage error (Price Panel): {e}")
...
def Get_60D_PriceHistory(stock_list, adjust_price = True, show_progress = True, save_excel = False, save_path = 'D:/FinPy-TSE Data/MarketWatch'):
    ...
    # DATABASE STORAGE: Store 60D Price History data in SQLite and PostgreSQL
    try:
        from database.models import RIHistory
        from database.sqlite_db import get_sqlite_session
        from database.postgres_db import get_postgres_session
        rihistory_records = []
        # hist_60_days: MultiIndex (Ticker, J-Date)
        for (ticker, jdate), row in hist_60_days.iterrows():
            rihistory_records.append(RIHistory(
                ticker=ticker,
                jdate=jdate,
                final=row.get('Final'),
                close=row.get('Close'),
                ...
                value=row.get('Value'),
                no=row.get('No'),
                yfinal=row.get('Y-Final')
            ))
        # Store in SQLite
        sqlite_session = get_sqlite_session()
        sqlite_session.bulk_save_objects(rihistory_records)
        sqlite_session.commit()
        sqlite_session.close()
        # Store in PostgreSQL
        postgres_session = get_postgres_session()
        postgres_session.bulk_save_objects(rihistory_records)
        postgres_session.commit()
        postgres_session.close()
    except Exception as e:
        print(f"Database storage error (60D Price History): {e}")
...
def Get_ShareHoldersInfo(ticker = 'خودرو'):
    ...
    """
    # find web-ids 
    ticker_no_df = get_tse_webid(ticker)
    if(type(ticker_no_df)==bool):
        return
    ...
    # DATABASE STORAGE: Store Shareholder Info in SQLite and PostgreSQL
    try:
        from database.models import Shareholder
        from database.sqlite_db import get_sqlite_session
        from database.postgres_db import get_postgres_session
        ...
        print(f"Database storage error (Shareholders): {e}")
